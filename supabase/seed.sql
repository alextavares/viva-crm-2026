...
begin
  ...
  values(v_owner,'authenticated','authenticated','owner@synthetic-a.invalid',extensions.crypt('SyntheticOnlyOwner',extensions.gen_salt('bf')),now(),'{}','{"full_name":"Synthetic Owner"}',now(),now())
  ...
  foreach v_role in array array['manager','broker','assistant'] loop
    ...
    v_id := ('00000000-0000-0000-0000-' || lpad((10 + array_position(array['manager','broker','assistant'],v_role))::text,12,'0'))::uuid;
    insert into auth.users(id,aud,role,email,encrypted_password,email_confirmed_at,raw_app_meta_data,raw_user_meta_data,created_at,updated_at)
    values(v_id,'authenticated','authenticated',v_email,extensions.crypt('SyntheticOnlyPassword',extensions.gen_salt('bf')),now(),'{}',jsonb_build_object('full_name','Synthetic '||initcap(v_role),'invite_token',v_token),now(),now()) on conflict (id) do nothing;
  end loop;

  insert into auth.users(id,aud,role,email,encrypted_password,email_confirmed_at,raw_app_meta_data,raw_user_meta_data,created_at,updated_at)
  values(v_tenant_b,'authenticated','authenticated','owner@synthetic-b.invalid',extensions.crypt('SyntheticOnlyOwnerB',extensions.gen_salt('bf')),now(),'{}','{"full_name":"Synthetic Owner B"}',now(),now()) on conflict (id) do nothing;
