-- Hero banner visual mode:
-- - compact (default): smaller section, better for short copy
-- - destaque: larger section, stronger visual emphasis
...
ALTER TABLE public.site_banners
ALTER COLUMN variant SET DEFAULT 'compact';

ALTER TABLE public.site_banners
ALTER COLUMN variant SET NOT NULL;

ALTER TABLE public.site_banners
DROP CONSTRAINT IF EXISTS site_banners_variant_check;

ALTER TABLE public.site_banners
ADD CONSTRAINT site_banners_variant_check
CHECK (variant IN ('compact', 'destaque'));

-- Ensure public payload includes banner variant.
