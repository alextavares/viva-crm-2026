 ...
 * URL esperada: POST /api/public/webhooks/[slug]/zap?token=xyz
 */
export async function POST(
    ...
    try {
        ...
        // 1. Validar a Organização e o Token do Zap
        ...
        console.log(`[Webhook Zap] Lead received for org ${slug}:`, JSON.stringify(payload));

        // 4. Extração Resiliente de Dados
        // O Zap/VivaReal ou o Zapier podem mandar de diferentes formas. Procuramos pelas chaves comuns.
        const castStr = (v: unknown) => (v ? String(v) : '');
        const name = castStr(payload.name || payload.nome || payload.firstName) || 'Lead Zap';
        ...
        if (!phone) {
            // Em leads imobiliários, telefone é obrigatório no nosso schema
            return NextResponse.json({ error: 'Missing phone number in payload' }, { status: 400 });
        }

        // Tentar encontrar o property_id no banco a partir do listingId (que é public_code ou o proprio UUID)
        let propertyId = null;
        if (listingIdRaw) {
        ...
        const { data: existingContact } = await supabaseAdmin
            ...
            .select('id, email')
            .eq('organization_id', orgId)
            .ilike('phone', `%${phoneNorm}%`) // Busca aproximada ou exata dependendo de como o tel está salvo
        ...
        if (existingContact) {
            ...
            // Atualizar e-mail se estiver faltando
            ...
            const { data: newContact, error: createError } = await supabaseAdmin
                ...
                .insert({
                    ...
                    status: 'new',
                    type: 'lead',
                    notes: `[Zap ${new Date().toISOString().split('T')[0]}] ${rawMessage}`.slice(0, 1000)
                })
                .select('id')
                .single();

            if (createError || !newContact) {
                console.error('[Webhook Zap] Error creating contact:', createError);
        ...
        console.error('[Webhook Zap] Unhandled error:', error);
