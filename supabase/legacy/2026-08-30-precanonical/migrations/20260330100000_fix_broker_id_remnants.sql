-- Fix remnants of broker_id that were missed in previous migrations

-- 1. Update goals_dashboard_snapshot to use assigned_to instead of broker_id
...
BEGIN
  ...
  IF v_effective_metric_cap_enabled THEN
    ...
    WHERE p.organization_id = v_org_id
      AND p.assigned_to = v_profile_id
  ...
  IF v_effective_metric_visitas_enabled THEN
    ...
    WHERE a.organization_id = v_org_id
      AND a.assigned_to = v_profile_id
...
-- 2. Ensure RLS policies don't contain broker_id
DO $$
BEGIN
    -- Appointments
